                                                        ...
                                                        foreach ($get_data as $data) {
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <a href="JavaScript:void(0);"  class=" edit m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="View">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#addRow").on("click", function (event) {
            var modal = bootbox.dialog({
                message: '<div class="form-group">' +
                        ...
                        '<textarea id="reference_range" name="reference_range">' +
                        '</div>',
                title: "<?= $model->title ?>",
                buttons: [
                    {
                        ...
                        callback: function () {
                            if ($('#name').val() === '')
                            {
                                ...
                                table.row.add([
                                    ...
                                    '<a href="JavaScript:Void(0);"  class=" trash m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="View">' +
                                            '<i class="la la-trash"></i>' +
                                            '</a><a href="JavaScript:Void(0);"  class=" edit m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill" title="View">' +
                                            '<i class="la la-edit"></i>' +
                                            '</a>',
                                ]).draw(false);
                            }
        ...
        $('#generate_lab tbody').on('click', '.edit', function (event) {
            var row = table.row($(this).parents('tr'));
            var header = table.$(this).parents('.group .group-start').find('td:first').text();
            //alert(header);
            var name = table.$(this).parents('tr').find('td:first').text();
            var unit = table.$(this).parents('tr').find('td:nth-child(3)').text();
            var range = table.$(this).parents('tr').find('td:nth-child(4)').html();

            var table_name = table.$(this).parents('tr').find('td:first');
            var table_unit = table.$(this).parents('tr').find('td:nth-child(3)');
            var table_range = table.$(this).parents('tr').find('td:nth-child(4)');

            //table.row($(this).parents('tr')).remove().draw();
            var modal = bootbox.dialog({
                message: '<div class="form-group">' +
                        '<label for="header">Header</label>' +
                        '<input type="text" name="header" id="header" value="' + header + '" class="form-control">' +
                        '</div>' +
                        '<div class="form-group">' +
                        '<label>Name</label>' +
                        '<input type="text" name="lab_name" id="name" value="' + name + '" class="form-control">' +
                        '</div>' +
                        '<div class="form-group">' +
                        '<label>Unit</label>' +
                        ...
                        '<div class="form-group">' +
                        '<label>Reference Range</label>' +
                        '<textarea name="reference_range" id="reference_range" class="form-control" rows="5">' + range + '</textarea>' +
                        '</div>',
                title: "<?= $model->title ?>",
                buttons: [
                    {
                        ...
                        callback: function () {
                            ...
                            if ($('#name').val() === '')
                            {
                                ...
                                temp[2] = $('#name').val();
                                temp[4] = $('#unit').val();
                                temp[5] = $('#reference_range').val();
                                //table.fnUpdate(temp, 5, undefined, false);
                               row
                                        .data(temp)
                                        .draw();
                              // table_name.text($('#name').val());
//                                table_unit.text($('#unit').val());
//                                table_range.html($('#reference_range').val());
                            }
                            modal.modal("hide");
                        }
                    },
...
//            $('#generate_lab').DataTable()
//    .clear()
//    .draw();
//            return '';
            console.log($('#generate_lab').DataTable().rows().data().toArray());
            var table = $('#generate_lab').DataTable();
