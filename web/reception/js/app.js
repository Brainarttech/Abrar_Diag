...
app.directive("hymn", function () {
  return {
    ...
    templateUrl: function (elem, attrs) {
      return baseUrl + "/reception/views/numericCalculator.html";
    },
  };
});
    ...
    return {
      ...
      link: function (scope, el, atts) {
        ...
        "columnDefs": [
          ...
          scope.directive.orderTable = orderTable;
        }, 1000);
      },
    };
  },
...
app.controller(
  ...
  function (
    ...
    $scope.search = function (mysearch) {
      $scope.directive.table.columns(1).search(mysearch).draw();
    };

    $scope.searchById = function (mysearch) {
      $scope.directive.table.columns(0).search(mysearch).draw();
    };
    ...
    $scope.addItemDiscount = function (data) {
      ...
      var modalInstance = $uibModal.open({
        ...
        templateUrl: baseUrl + "/reception/views/ItemDiscount.html",
    ...
    $rootScope.submit = function () {
      var Indata = {
        ...
        transaction_id: $rootScope.transaction_id,
        discount_type:
          $rootScope.discountType == 1
    ...
    $scope.discountDialogOptions = {
      templateUrl: baseUrl + "/reception/views/Discount.html",
      title: "Discount",
      scope: $scope,
      onEscape: function () {
        console.log("Closed");
    ...
    $scope.referredDialogOptions = {
      templateUrl: baseUrl + "/reception/views/Referred.html",
      title: "Referred",
      scope: $scope,
      onEscape: true,
    ...
    $scope.paymentDialogOptions = {
      templateUrl: baseUrl + "/reception/views/Payment.html",
      title: "Payment",
      scope: $scope,
      onEscape: true,
      closeButton: true,
    ...
    $scope.viewBillDialogOptions = {
      templateUrl: baseUrl + "/reception/views/ViewBill.html",
      title: "View Bill",
      scope: $scope,
      onEscape: true,
      closeButton: true,
    ...
    $scope.patientDialogOptions = {
      templateUrl: baseUrl + "/reception/views/Patient.html",
      title: "Patients",
      scope: $scope,
      onEscape: function () {
        console.log("Closed");
...
app.controller(
  ...
  function ($uibModalInstance, data, $rootScope, $scope) {
    //WaqarDiscount
    //console.log("ItemDiscountControoler"+data);
    var index = $rootScope.orderlist.indexOf(data);
    //console.log(index);
  ...
  function ($scope, $rootScope, $sce, $window, tabFocus, tabSelect) {
    ...
    var rsHtml = `
        <div class="form-group">
            ...
            <input
                ...
                ng-pattern="/(^[0-9]{1,9})+(\.[0-9]{1,4})?$/"
                ng-model="vm.discountRs"
                name="rs"
                class="form-control focus input-hike-large"
                format="decimal"
                selected="true"
            />
        </div>
        <div class="form-group">
            ...
            <select class="form-control" ng-model="$root.discountReason" name="rsReason">
                <option ng-repeat="product in reason" value="{{product.id}}" >
                    {{product.name}}
                </option>
            </select>
        </div>
    ...
    <div class="form-group">
        ...
        <input
            type="text"
            id="perdiscount"
            ng-model="vm.discountPer"
            floating-number-only
            ng-pattern="/(^[0-9]{1,9})+(\.[0-9]{1,4})?$/"
            name="percentage"
            class="form-control input-hike-large"
            format="decimal"
            selected="true"
        />
    </div>
    <div class="form-group" ng-if="$root.discountReason >= 10">
        <div class="margin-bottom-10">
            Transaction ID
        </div>
        <input
    ...
    <div class="form-group">
        ...
        <select class="form-control" ng-model="$root.discountReason" name="PerReason" ng-change="easyPaisa()">
            <option ng-repeat="product in reason" value="{{product.id}}" >
                {{product.name}}
            </option>
        </select>
    ...
    $scope.easyPaisa = function () {
      if ($rootScope.discountReason == 10) {
        $scope.vm.discountPer = 10;
      } else {
...
app.controller(
  ...
  function ($scope, $rootScope, $ngBootbox, $http) {
    $scope.$watch("referred_name", function (newValue, oldValue) {
      if ($scope.referred_name && $scope.referred_name.id) {
    ...
    $scope.referredDataSet = {
      ...
      limit: 10,
      source: function (query, syncResults, asyncResults) {
        clearTimeout(referredDoctorTimer);
        var ms = 100; // milliseconds
        referredDoctorTimer = setTimeout(function () {
          var req = {
...
app.controller(
  ...
  function (
    ...
    $scope.vm.easyPaisa = $rootScope.grandTotal;
    $scope.vm.jazzCash = $rootScope.grandTotal;

    var header = `<div class="form-group">
                    <div class="margin-bottom-10">Amount</div>`;
    ...
    var cash = `<input
        ...
        floating-number-only
        ng-model="vm.cash"
        my-enter="submitOrder()"
        name="cash"
        class="form-control focus input-hike-large text-right"
        id="cash"
    >`;
    var credit = `<input
        type="text"
        ...
        ng-model="vm.credit"
        name="credit"
        class="form-control input-hike-large text-right"
    >`;
    var bank = `<input
        type="text"
        floating-number-only
    ...
    $scope.Paymenttabs = [
      ...
      // { id:3,title:'Bank', content:$sce.trustAsHtml(header+bank+numericCalculator+fotter)},
      // { id:4,title:'Cheque', content:$sce.trustAsHtml(header+cheque+numericCalculator+fotter)},
      {
        id: 5,
        title: "E-Paisa",
        content: $sce.trustAsHtml(
          header + easyPaisa + numericCalculator + fotter
        ),
      },
      {
        id: 6,
        title: "J-Cash",
        content: $sce.trustAsHtml(
          header + jazzCash + numericCalculator + fotter
        ),
      },
    ];
    ...
    $scope.$watch("vm.cash", function (newValue, oldValue) {
      ...
      // console.log({type: 'cash', index: index, cash: newValue, mop: $rootScope.mop});
    ...
    $scope.$watch("vm.credit", function (newValue, oldValue) {
      ...
      // console.log({type: 'credit', index: index, cash: newValue, mop: $rootScope.mop});
      $scope.updatePayment(index, newValue);
    });

    $scope.$watch("vm.bank", function (newValue, oldValue) {
      var index = helper.getArrayIndex(3, $rootScope.mop);
      // console.log({type: 'bank', index: index, cash: newValue, mop: $rootScope.mop});
      $scope.updatePayment(index, newValue);
    });

    $scope.$watch("vm.cheque", function (newValue, oldValue) {
      var index = helper.getArrayIndex(4, $rootScope.mop);
      // console.log({type: 'cheque', index: index, cash: newValue, mop: $rootScope.mop});
      $scope.updatePayment(index, newValue);
    });

    $scope.$watch("vm.easyPaisa", function (newValue, oldValue) {
      var index = helper.getArrayIndex(5, $rootScope.mop);
      // console.log({type: 'cheque', index: index, cash: newValue, mop: $rootScope.mop});
      $scope.updatePayment(index, newValue);
    });

    $scope.$watch("vm.jazzCash", function (newValue, oldValue) {
      var index = helper.getArrayIndex(6, $rootScope.mop);
      // console.log({type: 'cheque', index: index, cash: newValue, mop: $rootScope.mop});
      $scope.updatePayment(index, newValue);
    });
    ...
    $scope.setActive = function (activePayment) {
      $rootScope.mop = [];
      switch (activePayment) {
        ...
        case 5:
          ...
          var easyPaisa = {
            name: "E-Paisa",
            id: activePayment,
            value: $scope.vm.easyPaisa,
          };
          $rootScope.mop.push(easyPaisa);
          $scope.sumRemaining();
          break;
        case 6:
          tabFocus("jazzCash");
          ...
          var jazzCash = {
            name: "J-Cash",
            id: activePayment,
            value: $scope.vm.jazzCash,
          };
          $rootScope.mop.push(jazzCash);
          $scope.sumRemaining();
          break;
        default:
          console.log("No FOCUS");
      }
      $scope.reCalculateTotal(activePayment);
    };
    ...
    $scope.reCalculateTotal = function (activePayment) {
      if (activePayment === 2) {
        $rootScope.tax = parseInt(($rootScope.total * 2.25) / 100);
      } else {
        $rootScope.tax = ($rootScope.total * 0) / 100;
      }
      if ($rootScope.testDiscount > 0) {
...
app.config(function ($routeProvider) {
  $routeProvider.when("/", {
    templateUrl: baseUrl + "/reception/views/Pos.html",
    controller: "POS",
  });
});
